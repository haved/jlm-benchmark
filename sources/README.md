...
The changes from actual SPEC2017 to `redist2017` are:
 ...
 - `500.perlbench` uses the original perl source code, without SPEC's modifications.
   It also skips a subset of the C files that are not generated by default.
 - `538.imagick` uses the original ImageMagick source code, without SPEC's modifications.
   All files are included, and configuration is set using the same options as in SPEC.
 - The rest of the bechmarks have their modified version available.
